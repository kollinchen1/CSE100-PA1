...
private:
  ...
  int heightHelper(BSTNode<Data>* n, int h) const;
 };
...
std::pair<BSTIterator<Data>, bool> BST<Data>::insert(const Data& item) 
{
  ...
  //Look for correct location to insert
  while(((item < curr->data) && (curr->left != NULL)) ||
        ((curr->data < item) && (curr->right != NULL)))
  {
    if ( item < curr->data )
    {
      curr = curr->left;
    ...
    {
      return std::pair<BSTIterator<Data>, bool>(BSTIterator<Data>(NULL), false);
    }
  } 
  
  //Ready to Insert
  //Check if data is duplicate, do it before mem-alloc to avoid hussle
...
BSTIterator<Data> BST<Data>::find(const Data& item) const
{
  ...
  return BSTIterator<Data>(returnVal);
}
...
/** Privte Height Helper for the BST 
 */
template <typename Data>
int BST<Data>::heightHelper(BSTNode<Data>* n, int h) const
{
  if( n == NULL)
    return h-1;
